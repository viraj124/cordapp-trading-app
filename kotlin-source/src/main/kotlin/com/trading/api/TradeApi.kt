...
class TradeApi(private val rpcOps: CordaRPCOps) {
    ...
    @PUT
    @Path("create-trade")
    fun createTrade(@QueryParam("sellValue") sellValue: Int,@QueryParam("sellCurrency") sellCurrency: String,@QueryParam("buyValue") buyValue: Int,@QueryParam("buyCurrency") buyCurrency: String, @QueryParam("counterParty") counterParty: CordaX500Name?,@QueryParam("tradeStatus") tradeStatus: String,@QueryParam("regulator") regulator: CordaX500Name?,@QueryParam("userId") userId: String,@QueryParam("assetCode") assetCode: String,@QueryParam("orderType") orderType: String,@QueryParam("transactionAmount") transactionAmount: Int,@QueryParam("transactionFees") transactionFees: Int,@QueryParam("transactionUnits") transactionUnits: Int): Response {

        if (counterParty == null) {
            return Response.status(BAD_REQUEST).entity("Query parameter 'Counter partyName' missing or has wrong format.\n").build()
        }
        if (regulator == null) {
            return Response.status(BAD_REQUEST).entity("Query parameter 'regulator' missing or has wrong format=.\n").build()
        }
        if (sellValue <= 0 ) {
        ...
        val regulator = rpcOps.wellKnownPartyFromX500Name(regulator) ?:
                return Response.status(BAD_REQUEST).entity("Regulatory named $regulator cannot be found.\n").build()

        return try {
            val signedTx = rpcOps.startTrackedFlowDynamic(TradeFlow.Initiator::class.java, sellValue,sellCurrency,buyValue,buyCurrency,tradeStatus,counterParty,regulator,userId,assetCode,orderType,transactionAmount,transactionFees,transactionUnits).returnValue.getOrThrow()
    ...
    fun counterTrade(@QueryParam("tradeId") tradeId: String,@QueryParam("sellValue") sellValue: Int,@QueryParam("sellCurrency") sellCurrency: String,@QueryParam("buyValue") buyValue: Int,@QueryParam("buyCurrency") buyCurrency: String, @QueryParam("counterParty") counterParty: CordaX500Name?,@QueryParam("tradeStatus") tradeStatus: String,@QueryParam("regulator") regulator: CordaX500Name, @QueryParam("userId") userId: String,@QueryParam("assetCode") assetCode: String,@QueryParam("orderType") orderType: String,@QueryParam("transactionAmount") transactionAmount: Int,@QueryParam("transactionFees") transactionFees: Int,@QueryParam("transactionUnits") transactionUnits: Int): Response {
        ...
        return try {
            val signedTx: SignedTransaction = rpcOps.startTrackedFlowDynamic(CounterTradeFlow.CounterInitiator::class.java, sellValue,sellCurrency,buyValue,buyCurrency,tradeStatus,tradeId,counterParty,regulator,userId,assetCode,orderType,transactionAmount,transactionFees,transactionUnits).returnValue.getOrThrow()
            Response.status(CREATED).entity("Transaction id ${signedTx.id} committed to ledger.\n").build()
